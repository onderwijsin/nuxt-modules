...
the `SLACK_BOT_TOKEN` and `SLACK_CHANNEL_ID` repository secrets to enable the notification. The
workflow sends the generated payload with Slack GitHub Action `v4.0.0` using the `chat.postMessage`
API method; the channel ID is included in the payload and the bot token is passed through the
action's `token` input.
