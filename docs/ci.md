...
The policy emits both an overall scope and enabled phases:

| Scope     | Meaning                             | Enabled phases                                                                             |
| --------- | ----------------------------------- | ------------------------------------------------------------------------------------------ |
| `light`   | No package behavior can be affected | format, lint                                                                               |
| `focused` | A package closure is affected       | format, lint, prepare, typecheck, test                                                     |
| `full`    | Repository-wide or uncertain impact | format, lint, prepare, typecheck, test, build, package validation, pack, external consumer |
...
validates those exact archives in a clean application. This intentionally duplicates some work while
build orchestration is changing so broken packages cannot pass solely because a previous artifact
was reused.
