...
describe("Directus typegen transforms", () => {
  ...
  it("falls back to an empty schema without credentials and fails partial production configuration", async () => {
    ...
    await expect(
      ...
      "requires both directusClient.instance.baseUrl and directusClient.client.typegen.introspectionToken."
    );
  ...
  it("skips type generation when explicitly disabled", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      "Skipping Directus type generation because directusClient.client.typegen.enabled is false."
    );
  });
