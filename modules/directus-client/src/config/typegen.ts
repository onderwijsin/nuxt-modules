 ...
 * This file is auto-generated by @onderwijsin/nuxt-directus-client.
...
export async function resolveDirectusTypegenDeclaration(options: {
  ...
  if (!hasAnyCredential) {
    ...
    if (isDefined(existing)) return existing;
    options.log.warn(
      "Skipping Directus type generation: configure directusClient.instance.baseUrl and directusClient.client.typegen.introspectionToken."
    );
    return "export interface Schema {}\n";
  }
  if (!options.directusUrl || !options.directusToken) {
    ...
    throw new Error(
      "Directus type generation requires both directusClient.instance.baseUrl and directusClient.client.typegen.introspectionToken."
    );
  }
...
export function applyTypegenTransforms(
  ...
  if (isDefined(options.transform)) {
    ...
    if (!isString(result)) {
      throw new Error("directusClient.client.typegen.transform must return a string declaration.");
    }
  }
...
function applyRules(
  ...
  for (const [collection, fields] of Object.entries(rules)) {
    ...
    if (!declaration) {
      throw new Error(
        `directusClient.client.typegen.rules references missing collection "${collection}".`
      );
    }
    ...
    for (const [fieldName, type] of Object.entries(fields)) {
      ...
      if (!field) {
        throw new Error(
          `directusClient.client.typegen.rules references missing field "${collection}.${fieldName}".`
        );
      }
      const colon = source.indexOf(":", field.nameStart);
      if (colon < 0 || colon > field.fieldEnd) continue;
