import {
  directusSerializableClientSchema,
...
export const directusClientOptionsSchema = directusConfigSchema
  .safeExtend({
    ...
    client: directusSerializableClientSchema.prefault({ auth: { maskSecretsInPlayground: true } })
  })
...
/** Resolved client boundary that retains executable shared-config functions. */
export const directusResolvedClientOptionsSchema = directusConfigSchema
  .safeExtend({
    enabled: enabled.default(true),
    instance: directusInstanceSchema.prefault({}),
...
export type ResolvedExecutableModuleOptions = z.output<typeof directusResolvedClientOptionsSchema>;
