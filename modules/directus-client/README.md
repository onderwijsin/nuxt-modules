...
| `client.auth.enabled`                 | `false`                             | Enables cookie authentication, authentication routes, and `useDirectusAuth`; requires `client.nuxtBaseUrl`. |
...
Configure `client.nuxtBaseUrl` with the absolute URL of the Nuxt application. Browser auth requests
remain relative same-origin proxy calls; SSR auth requests use this URL to reach the same proxy
handlers.
