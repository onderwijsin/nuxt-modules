...
describe("Directus module options", () => {
  it("allows enabled modules without a baseUrl", () => {
    expect(directusOptionsSchema.parse({})).toMatchObject({ enabled: true, instance: {} });
    expect(directusOptionsSchema.parse({ client: { typegen: { enabled: false } } })).toMatchObject({
      enabled: true,
      instance: {}
    });
  });
  ...
  it("defaults authentication Turnstile protection to disabled", () => {
    expect(directusOptionsSchema.parse({ client: { auth: { enabled: true } } })).toMatchObject({
      client: { auth: { enabled: true, turnstile: { enabled: false } } }
    });
    expect(
      directusOptionsSchema.parse({ client: { auth: { turnstile: { enabled: true } } } })
    ).toMatchObject({
  ...
  it("rejects root, traversal, and auth-colliding proxy paths", () => {
    for (const path of ["/", "/../proxy", "/_directus/auth/session"]) {
      expect(() => directusOptionsSchema.parse({ client: { proxy: { path } } })).toThrow();
    }
  });

  it("rejects non-local proxy paths", () => {
    expect(() =>
      directusOptionsSchema.parse({ client: { proxy: { path: "https://example.test" } } })
    ).toThrow();
  });
  ...
  it("uses the shared instance shape and rejects unknown direct module options", () => {
    expect(
      directusOptionsSchema.parse({ instance: { baseUrl: "https://directus.example.test" } })
    ).toMatchObject({ instance: { baseUrl: "https://directus.example.test" } });
    expect(() =>
      directusOptionsSchema.parse({ baseUrl: "https://directus.example.test" })
    ).toThrow();
    expect(() => directusOptionsSchema.parse({ auth: { enabled: true } })).toThrow();
    expect(() => directusOptionsSchema.parse({ instance: { baseUrl: "not-a-url" } })).toThrow();
  });
});
