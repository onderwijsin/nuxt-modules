...
including TTL. `removeItem(key)` removes the related internal records. Stale index records left by
expiry or interrupted writes are removed lazily during invalidation.
...
`kvApiToken` needs permission to delete keys from `cacheNamespaceId`. `clear()` is intentionally
custom only for the Cloudflare wrapper; the generic wrapper preserves every other driver's native
clear behavior. Both wrappers preserve `setItem()`, `setItems()`, `removeItem()`, `getMeta()`, and
`getKeys()` behavior, while hiding internal records from ordinary key listing.
...
| `targets[].match` | No       | `exact` (default) removes one route; `prefix` removes that route path and descendants. |
